...
enum nss_status internal_getent_r (void *result,
				   ...
				   int (*cmpFunc)(void *, void*))
{
	...
	while (1) {
		...
		do {
			...
			fgetpos (fpent->stream, &pos);
			buffer[buflen - 1] = '\xff';
			p = fgets_unlocked (buffer, buflen, fpent->stream);
			if (p == NULL && feof_unlocked (fpent->stream))
				return NSS_STATUS_NOTFOUND;

			if ((p == NULL) || (buffer[buflen - 1] != '\xff')) {
			erange_reset:
				fsetpos (fpent->stream, &pos);
				goto erange;
			}
			...
			buffer[buflen - 1] = '\0';
			  ...
			  !(parse_res = fpent->parser(p, result, data, buflen,
						      errnop)));

		if (parse_res == -1)
			goto erange_reset;

		if ( (cmpFunc == NULL) || (cmpFunc(result, cmpdata)))
			break;
	}
